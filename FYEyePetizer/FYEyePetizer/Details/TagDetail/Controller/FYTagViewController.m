...
@property (nonatomic, retain) FYTagSectionData *allData;
...
@implementation FYTagViewController
- (void)dealloc {
...
- (void)viewDidLoad {
    ...
    [self getDataWithTagId:_imageId strategy:@"date"];

}

- (void)getDataWithTagId:(NSNumber *)tagId strategy:(NSString *)strategy{
    ...
    NSString *urlString =[NSString stringWithFormat:@"http://baobab.wandoujia.com/api/v3/tag/videos?_s=%@&f=iphone&net=wifi&num=20&p_product=EYEPETIZER_IOS&start=0&strategy=%@&tagId=%@&u=227c329b8529f03c7ec60f7bba44edcfe0b12021&v=2.7.0&vc=1305", [timeStamp fy_stringByMD5Bit32], strategy, _imageId] ;
    [manager GET:urlString parameters:nil progress:nil success:^(NSURLSessionDataTask * _Nonnull task, id  _Nullable responseObject) {
        self.allData = [FYTagSectionData modelWithDic:responseObject];
...
- (void)segmentedControlValueChanged:(UISegmentedControl *)segmentedControl {
    if (0 == segmentedControl.selectedSegmentIndex) {
        ...
        [self getDataWithTagId:_imageId strategy:@"shareCount"];
    }
}
...
- (void)Loading{
    ...
    if ([_currentStrategy isEqualToString:@"date"]) {
        ...
        [manager GET:urlString parameters:nil progress:nil success:^(NSURLSessionDataTask * _Nonnull task, id  _Nullable responseObject) {
            FYTagSectionData *data = [FYTagSectionData modelWithDic:responseObject];
